...
abstract class CustomerSurveyViewModel extends State<CustomerSurveyScreen> {
      ...
      debugPrint('Kode ${question.id} : ${question.answer}');
        if(question.isRequired == '1'){
          if(question.type != 'checkbox'){
            ...
            if(question.multiAnswer.length == 0 || question.multiAnswer.length > question.maxCheck.toInt()){
              Fluttertoast.showToast(
                msg: 'Kode ${question.id} Tidak Boleh Kosong',
                gravity: ToastGravity.CENTER,
              );
        ...
        questionBody.add({
          ...
          'answer' : question.type != 'checkbox' ? question.answer : question.multiAnswer,
  ...
  List<Widget> generateInput(){
    ...
    listQuestion.forEach((question) {
      ...
      if(question.type.toString() == "choice") input = getRadioButton(question);
      if(question.type.toString() == "rating") input = getRatingField(question);
      if(question.type.toString() == "checkbox") input = getMultiChoiceField(question);
      _listInput.add(
        Card(
            elevation: 10,
                ...
                children: [
                  RichText(
                      text: TextSpan(children: [
                        TextSpan(
                            text: question?.question != null? '${question?.question} - Kode ${question.id}' : '',
          ...
          children: [
            ...(question?.optionList ?? []).map((e){
              return RaisedButton(
                ...
                color: (e.option == question.answer) ? MyColor.redAT : Colors.white,
                shape: RoundedRectangleBorder(
                    borderRadius: BorderRadius.circular(4.0),
                    side: BorderSide(color:  (e.option == question.answer) ? Colors.white : MyColor.redAT )),
                child: Text(
                  e.option,
                  style: TextStyle(color: (e.option == question.answer) ? Colors.white : MyColor.redAT ),
                ),
                // child: Text(e.jawaban_id ?? ''),
              );
            }).toList(),
        ],
      ),
        ...
        children: [
          ...(question?.optionList ?? []).map((e){
            return RaisedButton(
              onPressed: () {
                setState(() {
                  ...
                  if(question.multiAnswer.contains(e.option))
                    question.multiAnswer.removeWhere((ans) => ans == e.option);
                  else{
                    if(question.multiAnswer.length < 3){
                      question.multiAnswer.add(e.option);
                    }else{
                      Get.defaultDialog(title: "", content: Text("Maksimal ${question.maxCheck}"));
                    }
                  }
              ...
              color: (question.multiAnswer.contains(e.option)) ? MyColor.redAT : Colors.white,
              shape: RoundedRectangleBorder(
                  borderRadius: BorderRadius.circular(4.0),
                  side: BorderSide(color:  (question.multiAnswer.contains(e.option)) ? Colors.white : MyColor.redAT )),
              child: Text(
                e.option,
                style: TextStyle(color: (question.multiAnswer.contains(e.option)) ? Colors.white : MyColor.redAT ),
              ),
              // child: Text(e.jawaban_id ?? ''),
            );
          }).toList(),
